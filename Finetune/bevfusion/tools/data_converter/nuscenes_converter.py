...
def _get_description(nusc, sample):
    scene_token = sample['scene_token']
    sample2scene = nusc.get('scene', scene_token)
    description = sample2scene['description']

    ret_time = re.search("Night", description)
    ret_weather = re.search("(?<!after )[Rr]ain", description)

    weather = np.array([0, 0, 0, 0])
    rainy = np.array([0, 0, 0, 1])
    night = np.array([0, 1, 0, 0])
    weather = weather | rainy if ret_weather else weather
    weather = weather | night if ret_time else weather
    return weather
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, max_sweeps=10):
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        weather = _get_description(nusc, sample)
