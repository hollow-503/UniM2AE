...
def single_gpu_test(model,
    ...
    os.makedirs(os.path.join(out_dir, 'chamf'), exist_ok=True)

    for i, data in enumerate(data_loader):
        cam_out = osp.join(cam_out_dir, f'cam_{i}')
        with torch.no_grad():
            result = model(return_loss=False, rescale=True, pretrain=show_pretrain, out_dir=cam_out, **data)
        ...
        if show_pretrain:
            ...
            if result["points"] is not None:
                ...
                for b in range(batch_size):
                    ...
                    plt.savefig(f"{out_dir}/chamf/chamf_points_bev{i}_{b}.png")
                    plt.close()
